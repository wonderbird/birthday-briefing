...
- Frontend framework:
  - React (planned) for building the user interface and managing component state.
- Styling:
  - Standard web technologies (HTML, CSS, and JavaScript) with React.
- Data storage:
  - Browser local storage for:
    - User configuration (CalDAV URL, first day of week).
    - Cached birthday data needed for the 14-day view.
- External integration:
  - CalDAV-compatible server as the single external data source for birthday information.
...
- Privacy requirements:
  ...
  - All sensitive data remains in the user’s browser and in the user’s chosen CalDAV system.
...
- Testability (conceptual):
  - Structure code so that data processing (for example, transforming CalDAV events into birthdays within a 14-day window) can be tested without depending on the browser UI.
...
- Provides the technical foundation for the patterns outlined in `systemPatterns.md`:
  ...
  - Local storage and CalDAV integration.
